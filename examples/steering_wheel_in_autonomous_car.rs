...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut steering = LinguisticVariable::new("steering", Range { min: -30.0, max: 30.0 });
    // TODO: add output definitions

    // Input: lane deviation (meters from center)
    let mut deviation = LinguisticVariable::new("deviation", Range { min: -2.0, max: 2.0 });
    // TODO: add input definitions

    // Input: road curvature (negative = left curve, positive = right curve)
    let mut curvature = LinguisticVariable::new("curvature", Range { min: -1.0, max: 1.0 });
    // TODO: add input definitions
