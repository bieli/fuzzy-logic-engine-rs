...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut irrigation = LinguisticVariable::new("irrigation", Range { min: 0.0, max: 100.0 });
    // TODO: add output definition

    // Input: soil moisture (% volumetric water content)
    let mut soil = LinguisticVariable::new("soil_moisture", Range { min: 0.0, max: 100.0 });
    // TODO: add input definitions

    // Input: rain forecast (mm expected in next 24h)
    let mut rain = LinguisticVariable::new("rain_forecast", Range { min: 0.0, max: 50.0 });
    // TODO: add input definitions

    // Input: crop growth stage (0 = germination, 10 = harvest)
    let mut stage = LinguisticVariable::new("growth_stage", Range { min: 0.0, max: 10.0 });
    // TODO: add input definitions
